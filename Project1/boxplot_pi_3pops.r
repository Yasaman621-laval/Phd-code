...
cat(">>> Running Step 5, scenarioIndex =", scenarioIndex, "\n")

# ============================================================
# 2) Detect scenario directory (3-pops)
# ============================================================
...
if (scenarioIndex > length(scenario_dirs)) {
  stop("scenarioIndex out of range.")
}
...
# ============================================================
# 3) Extract hsq and rho from scenario name
# Example: sim_hsq0.5_rho0.3_trainEUR80000
# ============================================================
...
# ============================================================
# 4) Load TRUE p-values from simulation summary
# ============================================================
summary_file <- paste0(dirBase, "SimulationSummary_realisticScenarios.csv")
df <- read.csv(summary_file)

get_pi_values <- function(df, hsq_value, rho_value) {
    row <- df[df$hsq == hsq_value & df$rho == rho_value, ]
    if (nrow(row) == 0) stop("No matching scenario in SimulationSummary!")
...
# ============================================================
# 5) Load es-alpha best rows for sim = 1..10
# ============================================================
sim_list <- list()
...
for (sim in 1:10) {
    ...
    if (length(files) == 0) {
        cat(">>> WARNING: Missing es-alpha for sim", sim, "\n")
        next
    }

    load(files[1])  # loads: es_alphaMatrix, row_index, tauuse, selected_row

    # double check selected_row exists:
    if (!exists("selected_row")) {
...
# Remove NULL entries (in case some sims missing)
sim_list <- sim_list[!sapply(sim_list, is.null)]

if (length(sim_list) == 0) stop("No es-alpha files found for this scenario!")

sim_mat <- do.call(rbind, sim_list)
colnames(sim_mat) <- c("pi_0","pi_1","pi_2","pi_3","pi_12","pi_13","pi_23","pi_123")

sim_df <- data.frame(sim = seq_len(nrow(sim_mat)), sim_mat)
...
p <- ggplot(data_long, aes(Parameter, Value)) +
  ...
  "hsq = ", hsq, ",  rho = ", rho, "  —  Three Populations"
))+
    theme(
...
out_plot <- paste0(file.rjobs, "/pi_boxplot_3pops_scenario", scenarioIndex, ".png")
ggsave(out_plot, p, width = 10, height = 6, dpi = 300)
...
write.csv(summary_table,
          file = paste0(file.rjobs, "/summary_table3pops_scenario", scenarioIndex, ".csv"),
          row.names = FALSE)
