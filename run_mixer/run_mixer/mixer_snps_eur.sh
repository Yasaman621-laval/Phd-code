...
#SBATCH --job-name=mixer_snps_EUR
#SBATCH --ntasks=1
...
echo "[$(date)] Generating EUR .snps for chr${CHR}"

srun python3 "${MIXER_PY}" snps \
  --lib "${LIBBG}" \
  --bim-file "${REFDIR}/EURchr${CHR}bedformat.bim" \
  --ld-file  "${LD_DIR}/chr${CHR}.EUR.ld" \
  --chr2use  "${CHR}" \
  --out      "${LD_DIR}/EUR_chr${CHR}.prune_maf0p05_rand2M_r2p8.snps" \
  --maf 0.05 \
  --r2 0.8 


echo "[$(date)] Done EUR .snps for chr${CHR}"
